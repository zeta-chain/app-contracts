...
contract Disperse {
    ...
    event disperse(address indexed token, address indexed recipient, uint256 value);
    ...
    function disperseEther(address[] calldata recipients, uint256[] calldata values) external payable noReentrancy {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            emit disperse(address(0), recipients[i], values[i]);
        }
    ...
    function disperseToken(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            emit disperse(address(token), recipients[i], values[i]);
        }
    }
