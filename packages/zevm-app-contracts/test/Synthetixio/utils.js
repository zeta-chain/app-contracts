...
module.exports = () => {
  ...
  const assertEventsEqual = (transaction, ...expectedEventsAndArgs) => {
    ...
    if (expectedEventsAndArgs.length <= 2)
      throw new Error("Expected events and args can be called with just assert.eventEqual as there's only one event.");
  ...
  const assertBNClose = (actualBN, expectedBN, varianceParam = "10") => {
    ...
    assert.ok(actual.gte(expected.sub(variance)), `${actual} !~= ${expected} (maxVariance ${variance.toString()})`);
    assert.ok(actual.lte(expected.add(variance)), `${actual} !~= ${expected} (maxVariance ${variance.toString()})`);
  };
  ...
  const assertInvalidOpcode = async blockOrPromise => {
    ...
    assert.strictEqual(errorCaught, true, "Operation did not cause an invalid opcode error as expected");
  };
